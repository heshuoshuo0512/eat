...
DROP POLICY IF EXISTS pilot_invitations_consume ON pilot_invitations;
CREATE POLICY pilot_invitations_consume ON pilot_invitations
  FOR UPDATE USING (
    app_tenant_matches(tenant_id)
    AND (app_current_role() = 'authenticator' OR app_can_manage_users())
  ) WITH CHECK (
...
CREATE POLICY pilot_invitations_manage ON pilot_invitations
  FOR INSERT WITH CHECK (app_tenant_matches(tenant_id) AND app_can_manage_users());

DROP POLICY IF EXISTS pilot_invitations_revoke ON pilot_invitations;
CREATE POLICY pilot_invitations_revoke ON pilot_invitations
  FOR DELETE USING (app_tenant_matches(tenant_id) AND app_can_manage_users());

INSERT INTO schema_migrations(version) VALUES ('029_pilot_invitations')
