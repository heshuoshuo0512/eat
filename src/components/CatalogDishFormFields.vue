<template>
  <div :class="['catalog-dish-fields', { compact }]">
    <fieldset class="dish-field-group">
      <legend>基础信息</legend>
      <label>所属档口
        <select v-model="form.stallId" required @change="$emit('stall-change')">
      ...
      <label>菜名<input v-model.trim="form.name" required maxlength="40" /></label>
      <div class="dish-field-grid two">
        <label>价格<input v-model.number="form.price" type="number" min="1" max="200" step="0.1" required /></label>
        <label>口味<input v-model.trim="form.taste" required maxlength="20" /></label>
        <label>菜系<input v-model.trim="form.cuisine" required maxlength="30" /></label>
        <label>餐次<input v-model.trim="form.mealTypes" placeholder="lunch, dinner" /></label>
      </div>
      <label>食材<input v-model="form.ingredients" required placeholder="鸡肉, 米饭, 西兰花" /></label>
      <label>标签<input v-model="form.tags" required placeholder="高蛋白, 低脂" /></label>
    </fieldset>

    <fieldset class="dish-field-group">
      <legend>营养与安全</legend>
      <div class="dish-field-grid nutrition">
        <label>热量<input v-model.number="form.calories" type="number" min="0" /></label>
        <label>蛋白质<input v-model.number="form.protein" type="number" min="0" /></label>
        <label>脂肪<input v-model.number="form.fat" type="number" min="0" /></label>
        <label>碳水<input v-model.number="form.carbs" type="number" min="0" /></label>
      </div>
      <label class="dish-switch-row"><input v-model="form.halal" type="checkbox" /><span>清真菜品</span></label>
      <label>过敏原<input v-model="form.allergens" placeholder="花生, 牛奶" /></label>
    </fieldset>

    <details class="dish-advanced-fields">
      <summary>扩展营养、图片与展示</summary>
      <div class="dish-field-grid nutrition">
        <label>膳食纤维<input v-model.number="form.fiber" type="number" min="0" /></label>
        <label>钠<input v-model.number="form.sodium" type="number" min="0" /></label>
        <label>糖<input v-model.number="form.sugar" type="number" min="0" /></label>
        <label>钙<input v-model.number="form.calcium" type="number" min="0" /></label>
        <label>铁<input v-model.number="form.iron" type="number" min="0" step="0.1" /></label>
      </div>
      <div class="dish-field-grid two">
        <label>展示状态
      ...
      <label>描述<textarea v-model.trim="form.description" rows="4" maxlength="300"></textarea></label>
