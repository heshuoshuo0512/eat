...
const store=useCanteenStore();const targetOptions=[{value:'dish',label:'关联菜品'},{value:'canteen',label:'关联场所'}];const form=reactive({targetType:'dish',content:'',rating:0});const canteenId=ref('');const stallId=ref('');const dishId=ref('');const canteenQuery=ref('');const stallQuery=ref('');const dishQuery=ref('');const imagePath=ref('');const imageSize=ref(0);const submitting=ref(false);const message=ref('');const isError=ref(false);const availableDishes=ref([]);const dishesLoading=ref(false);const PUBLISH_DRAFT_KEY='smart-canteen-community-publish-draft';let searchTimer;
...
onShow(async()=>{try{await store.refreshIfStale();if(!store.user.value){uni.reLaunch({url:'/pages/login/login'});return;}restoreDraft();}catch{}});watch(()=>form.targetType,()=>{canteenId.value='';stallId.value='';dishId.value='';canteenQuery.value='';stallQuery.value='';dishQuery.value='';availableDishes.value=[];form.rating=0;});
...
function saveDraft(){uni.setStorageSync(PUBLISH_DRAFT_KEY,{form:{...form},canteenId:canteenId.value,stallId:stallId.value,dishId:dishId.value,canteenQuery:canteenQuery.value,stallQuery:stallQuery.value,dishQuery:dishQuery.value,imagePath:imagePath.value,imageSize:imageSize.value});}
function restoreDraft(){const draft=uni.getStorageSync(PUBLISH_DRAFT_KEY);if(!draft)return;Object.assign(form,draft.form||{});canteenId.value=draft.canteenId||'';stallId.value=draft.stallId||'';dishId.value=draft.dishId||'';canteenQuery.value=draft.canteenQuery||'';stallQuery.value=draft.stallQuery||'';dishQuery.value=draft.dishQuery||'';imagePath.value=draft.imagePath||'';imageSize.value=Number(draft.imageSize||0);if(stallId.value)loadDishOptions();}
async function submit(){const targetId=form.targetType==='dish'?dishId.value:canteenId.value;if(!targetId||form.content.trim().length<2){message.value='请选择关联对象，并填写至少 2 个字符。';isError.value=true;return;}if(!store.user.value){saveDraft();uni.reLaunch({url:'/pages/login/login'});return;}if(!store.user.value.profileComplete){saveDraft();uni.setStorageSync('community-publish-resume','1');uni.switchTab({url:'/pages/profile/profile'});return;}submitting.value=true;message.value='';isError.value=false;try{let imageUrl='';if(imagePath.value){const extension=String(imagePath.value).split('.').pop()?.toLowerCase();const contentType=extension==='png'?'image/png':extension==='webp'?'image/webp':'image/jpeg';const upload=await store.uploadImage({filename:`campus-post.${extension||'jpg'}`,contentType,dataBase64:await imageToBase64(imagePath.value),sizeBytes:imageSize.value});imageUrl=upload.reference||upload.url||upload.publicUrl||'';}const result=await store.createPost({targetType:form.targetType,targetId,content:form.content.trim(),imageUrl,rating:form.targetType==='dish'&&form.rating?form.rating:null});store.openCommunitySection('posts');uni.removeStorageSync(PUBLISH_DRAFT_KEY);const moderation=result.moderation||{};message.value=moderation.status==='rejected'?'内容未通过自动审核':moderation.status==='pending'?'内容正在审核':'帖子已公开';isError.value=moderation.status==='rejected';if(moderation.status!=='rejected')setTimeout(()=>uni.navigateBack(),700);}catch(error){if(error.statusCode===428||error.code==='PROFILE_REQUIRED'){saveDraft();uni.setStorageSync('community-publish-resume','1');uni.switchTab({url:'/pages/profile/profile'});return;}message.value=error.message||'发布失败';isError.value=true;}finally{submitting.value=false;}}
</script>
