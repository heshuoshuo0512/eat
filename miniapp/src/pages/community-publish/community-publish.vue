  ...
  <sc-page-shell back title="发布帖子" subtitle="关联真实食堂或菜品" tone="community">
    <view class="publish-form panel-card">
      <view class="form-main">
        ...
        <label v-if="form.targetType==='dish'"><text>菜品</text><picker :disabled="dishesLoading" :range="availableDishes" range-key="name" :value="dishIndex" @change="selectDish"><view class="picker-box">{{ dishesLoading?'正在读取菜品':selectedDish?.name||'请选择菜品' }}<text>⌄</text></view></picker></label>
...
const store=useCanteenStore();const targetOptions=[{value:'dish',label:'关联菜品'},{value:'canteen',label:'关联场所'}];const form=reactive({targetType:'dish',content:'',rating:0});const canteenId=ref('');const stallId=ref('');const dishId=ref('');const imagePath=ref('');const imageSize=ref(0);const submitting=ref(false);const message=ref('');const isError=ref(false);const availableDishes=ref([]);const dishesLoading=ref(false);
const selectedCanteen=computed(()=>store.canteens.value.find((item)=>item.id===canteenId.value));const canteenIndex=computed(()=>Math.max(0,store.canteens.value.findIndex((item)=>item.id===canteenId.value)));const availableStalls=computed(()=>store.stalls.value.filter((item)=>item.canteenId===canteenId.value));const selectedStall=computed(()=>availableStalls.value.find((item)=>item.id===stallId.value));const stallIndex=computed(()=>Math.max(0,availableStalls.value.findIndex((item)=>item.id===stallId.value)));const selectedDish=computed(()=>availableDishes.value.find((item)=>item.id===dishId.value));const dishIndex=computed(()=>Math.max(0,availableDishes.value.findIndex((item)=>item.id===dishId.value)));
...
function selectCanteen(event){canteenId.value=store.canteens.value[Number(event.detail.value)]?.id||'';stallId.value='';dishId.value='';availableDishes.value=[];}function selectStall(event){stallId.value=availableStalls.value[Number(event.detail.value)]?.id||'';dishId.value='';loadDishOptions();}function selectDish(event){dishId.value=availableDishes.value[Number(event.detail.value)]?.id||'';}
async function loadDishOptions(){if(!stallId.value){availableDishes.value=[];return;}dishesLoading.value=true;try{const result=await store.communityDishOptions({stallId:stallId.value,page:1,pageSize:100});availableDishes.value=result.options||[];}catch(error){availableDishes.value=[];message.value=error.message||'菜品目录读取失败';isError.value=true;}finally{dishesLoading.value=false;}}
