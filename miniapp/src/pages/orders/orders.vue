<template>
  <sc-page-shell back title="到店预约" subtitle="同档口预约，到店确认并支付" tone="meal">
    ...
    <template v-if="activePanel==='menu'">
      ...
      <view class="menu-head"><view><text>稳定目录</text><text class="ui-strong">校园菜单</text></view><button :loading="menuLoading" @tap="reloadMenu">刷新</button></view>
      <sc-state-card v-if="menuLoading&&!menuDishes.length" type="loading" title="正在读取校园菜单" />
      ...
      <view v-else class="menu-list">
        ...
        <sc-state-card v-if="!menuDishes.length" type="empty" title="目录暂无可预约菜品" desc="请检查网络后重试，系统不会显示假数据。" />
        <text v-if="store.reservationCatalogPage.value.hasMore" class="catalog-progress">{{ loadingMoreMenu?'正在加载更多':`已加载 ${menuDishes.length} / ${store.reservationCatalogPage.value.total}，继续上拉` }}</text>
      </view>
        </view>

      <view class="cart-panel">
        <view class="cart-title"><view><text>预约清单</text><text class="ui-strong">{{ cartCount }} 份 · 预计 ¥{{ cartAmount }}</text></view><button v-if="cart.length" @tap="clearCart">清空</button></view>
        ...
        <text v-else class="cart-empty">从校园菜单选择同一档口菜品。</text>
        <button class="submit-disabled" :disabled="!cart.length||submitting" @tap="submitReservation">{{ submitting?'提交中…':'提交预约 · 到店支付' }}</button>
      </view>
      </view>
    </template>
    ...
    <template v-else>
      ...
      <view v-else class="order-list">
        <view v-for="order in orders" :key="order.id" class="order-card">
          <view class="order-top"><view><text class="ui-strong">{{ statusLabel(order.status) }}</text><text>{{ formatTime(order.createdAt) }}</text></view><text>到店支付</text></view>
          <view class="order-items"><text v-for="item in order.items" :key="item.id">{{ item.dishName }} × {{ item.quantity }}</text></view>
          <view class="pickup-row"><view><text>预约码</text><text class="ui-strong">{{ order.pickupCode||'待生成' }}</text></view><button :disabled="!order.pickupCode" @tap="copyCode(order.pickupCode)">复制</button><text class="ui-bold">{{ order.pricingStatus==='pending_confirmation'?'预计 ':'' }}¥{{ Number((order.finalAmount??order.estimatedAmount)||0).toFixed(2) }}</text></view>
        </view>
        <sc-state-card v-if="!orders.length" type="empty" illustration="order-complete" title="还没有预约记录" desc="提交后会在这里显示预约码和档口状态。" />
      </view>
    </template>
  </sc-page-shell>
</template>
...
const CART_KEY='smart-canteen-reservation-cart';
const store=useCanteenStore();const panelOptions=[{value:'menu',label:'校园菜单'},{value:'history',label:'预约记录'}];
const activePanel=ref('menu');const cart=ref([]);const orders=ref([]);const cartPulseId=ref('');const menuLoading=ref(false);const loadingMoreMenu=ref(false);const menuError=ref('');const ordersLoading=ref(false);const ordersError=ref('');const submitting=ref(false);let requestedDishId='';let initialized=false;let cartPulseTimer=0;
const menuDishes=computed(()=>store.todayMenu.value.dishes||[]);const cartCount=computed(()=>cart.value.reduce((sum,item)=>sum+Number(item.quantity||0),0));const cartAmount=computed(()=>cart.value.reduce((sum,item)=>sum+Number(item.price||0)*Number(item.quantity||0),0).toFixed(2));
...
onReachBottom(async()=>{if(activePanel.value!=='menu'||loadingMoreMenu.value||!store.reservationCatalogPage.value.hasMore)return;loadingMoreMenu.value=true;try{await store.loadMoreTodayMenu();}catch(error){menuError.value=error.message||'加载更多失败。';}finally{loadingMoreMenu.value=false;}});
...
function addRequestedDish(){if(!requestedDishId)return;const dish=menuDishes.value.find((item)=>String(item.id)===requestedDishId);if(dish&&supplyState(dish).canAdd){add(dish);uni.showToast({title:'已加入预约清单',icon:'none'});}else uni.showToast({title:'该菜当前不可预约',icon:'none'});requestedDishId='';}
function add(dish){if(!supplyState(dish).canAdd)return;if(cart.value.length&&String(cart.value[0].stallId)!==String(dish.stallId)){uni.showToast({title:'一张预约单只能选择同一档口',icon:'none'});return;}const existing=cart.value.find((item)=>String(item.dishId)===String(dish.id));if(existing)existing.quantity+=1;else cart.value.push({dishId:dish.id,name:dish.name,price:Number((dish.pricing?.minAmount??dish.price)||0),priceDisplay:dish.priceDisplay||'',stallId:dish.stallId,quantity:1});persist();cartPulseId.value=String(dish.id);clearTimeout(cartPulseTimer);cartPulseTimer=setTimeout(()=>{cartPulseId.value='';},420);triggerHaptic('light');}
...
function supplyState(dish){if(dish.availability?.orderable===false)return{label:'暂停预约',className:'sold',canAdd:false};return{label:'可预约',className:'available',canAdd:true};}
...
async function reloadMenu(){menuLoading.value=true;menuError.value='';try{await store.loadTodayMenu();}catch(error){menuError.value=error.message||'校园菜单加载失败。';}finally{menuLoading.value=false;}}
async function submitReservation(){if(!cart.value.length||submitting.value)return;submitting.value=true;try{const result=await store.createOrder({items:cart.value.map((item)=>({dishId:item.dishId,quantity:item.quantity}))});cart.value=[];persist();triggerHaptic('success');uni.showModal({title:'预约成功',content:`预约码 ${result.order.pickupCode}\n请到档口确认价格并支付`,showCancel:false});activePanel.value='history';await loadOrders();}catch(error){uni.showToast({title:error.message||'预约提交失败',icon:'none'});}finally{submitting.value=false;}}
...
function openDish(id){uni.navigateTo({url:`/pages/dish-detail/dish-detail?id=${encodeURIComponent(id)}`});}function copyCode(code){if(!code)return;uni.setClipboardData({data:String(code),success:()=>uni.showToast({title:'预约码已复制',icon:'none'})});}
function statusLabel(value){return{pending:'待接单',preparing:'准备中',ready:'可到店',completed:'已完成',cancelled:'已取消'}[value]||value||'处理中';}function formatTime(value){return String(value||'').replace('T',' ').slice(0,16);}
