...
async function savePublicProfile(){profileSaving.value=true;try{await store.updatePublicProfile({nickname:nickname.value,avatarUrl:avatarReference.value||store.user.value?.avatarUrl});uni.showToast({title:'资料已保存',icon:'success'});if(uni.getStorageSync('community-publish-resume')){uni.removeStorageSync('community-publish-resume');setTimeout(()=>uni.navigateTo({url:'/pages/community-publish/community-publish'}),250);}}catch(error){uni.showToast({title:error?.message||'保存失败',icon:'none'});}finally{profileSaving.value=false;}}
