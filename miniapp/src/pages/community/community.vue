...
const section=ref('posts');const postType=ref('');const postQuery=ref('');const posts=ref([]);const postTotal=ref(0);const reviews=ref([]);const reviewQuery=ref('');const reviewTotal=ref(0);const reviewSummary=ref({});const loading=ref(false);const loadingMore=ref(false);const error=ref('');const filtersOpen=ref(false);const pageSize=20;const dishOptionItems=ref([]);const canteenFilterQuery=ref('');const stallFilterQuery=ref('');const dishFilterQuery=ref('');const openCommentsId=ref('');const commentsLoading=ref(false);const commentsByPost=reactive({});const commentDrafts=reactive({});
...
let ready=false;let postSearchTimer=0;let reviewSearchTimer=0;let dishFilterTimer=0;let lastLoadedAt=0;let syncingSection=false;
...
async function loadCurrent(reset){return section.value==='posts'?loadPosts(reset):loadReviews(reset);}async function loadPosts(reset=false){if(reset){loading.value=true;posts.value=[];}else loadingMore.value=true;error.value='';try{const data=await store.listPosts({targetType:postType.value==='mine'?'':postType.value,mine:postType.value==='mine',q:postQuery.value.trim(),limit:pageSize,offset:reset?0:posts.value.length});posts.value=reset?(data.posts||[]):[...posts.value,...(data.posts||[])];postTotal.value=Number(data.total||0);lastLoadedAt=Date.now();}catch(err){error.value=err.message||'帖子加载失败';}finally{loading.value=false;loadingMore.value=false;}}
