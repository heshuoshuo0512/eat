...
const section=ref('posts');const postType=ref('');const postQuery=ref('');const posts=ref([]);const postTotal=ref(0);const reviews=ref([]);const reviewQuery=ref('');const reviewTotal=ref(0);const reviewSummary=ref({});const loading=ref(false);const loadingMore=ref(false);const error=ref('');const filtersOpen=ref(false);const pageSize=20;
