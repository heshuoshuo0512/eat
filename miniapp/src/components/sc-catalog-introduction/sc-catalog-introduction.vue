<template>
  <view v-if="summary" class="catalog-introduction" :class="{ compact }" @tap.stop>
    <text class="summary">{{ summary }}</text>
    <view v-if="introduction?.recommendationCopy" class="recommendation">
      <button class="toggle" @tap.stop="expanded=!expanded">
        <text>{{ introduction.provenanceLabel||'基于目录整理' }}</text>
        <text>{{ expanded?'收起':'展开' }}</text>
      </button>
      <text v-if="expanded" class="copy">{{ introduction.recommendationCopy }}</text>
    </view>
...
const props=defineProps({entity:{type:Object,default:()=>({})},compact:Boolean});
